...
void MangaHere::decryptListOfTitles() {
    qDebug() << "decryptListOfTitles" << reply;
    reply->deleteLater();
    ...
    qDebug() << "data size" << contents.length();
    //qDebug() << contents;
    mainWindow->startAction(tr("Decypher list of titles"));
    int pos = 0;
    while ((pos = re.indexIn(contents, pos)) != -1) {
        ...
        mainWindow->setProgress(pos, contents.length());
    }
    qDebug() << "read " << comicsData.keys().count();
